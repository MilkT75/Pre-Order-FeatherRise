...
// ใช้ Config ของคุณที่ให้มาในตอนแรก
...
export default function App() {
  ...
  useEffect(() => {
    // ล็อกอินแบบไม่ต้องระบุตัวตนเพื่อให้เขียนอ่านข้อมูลได้ (ตั้ง Rule ใน Firebase ให้รองรับด้วย)
  ...
  useEffect(() => {
    ...
    // Fetch Orders (เปลี่ยน Path เป็นมาตรฐาน)
    ...
    // Fetch Settings (เปลี่ยน Path เป็นมาตรฐาน)
    ...
    <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="bg-white rounded-[32px] p-8 w-full max-w-sm text-center shadow-2xl animate-in zoom-in-95 duration-200">
        <div className="flex justify-between items-center mb-6">
          ...
          <button onClick={() => setShowAdminLogin(false)} className="w-8 h-8 flex items-center justify-center text-gray-400 bg-gray-50 rounded-full active:scale-90">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="flex justify-center gap-3 mb-8">
          {[0, 1, 2, 3, 4, 5].map(i => (
            <div key={i} className={`w-3.5 h-3.5 rounded-full border-2 transition-all duration-200 ${i < pinInput.length ? 'bg-black border-black' : 'border-gray-200'}`} />
          ))}
        </div>

        <div className="grid grid-cols-3 gap-3">
          {[1, 2, 3, 4, 5, 6, 7, 8, 9].map(n => (
            <button key={n} onClick={() => handlePinPress(n.toString())} className="h-14 bg-gray-50 hover:bg-gray-100 active:bg-gray-200 rounded-2xl text-xl font-medium text-gray-900 transition-all active:scale-95">
              {n}
            </button>
          ))}
          <div />
          <button onClick={() => handlePinPress('0')} className="h-14 bg-gray-50 hover:bg-gray-100 active:bg-gray-200 rounded-2xl text-xl font-medium text-gray-900 transition-all active:scale-95">0</button>
          <button onClick={() => handlePinPress('del')} className="h-14 bg-gray-50 hover:bg-gray-100 active:bg-gray-200 rounded-2xl text-xl font-medium text-gray-500 transition-all active:scale-95 flex items-center justify-center">
            ⌫
          </button>
        </div>
  ...
  if (!settings && role === 'guest') {
    return <div className="min-h-screen bg-[#FDFDFD] flex items-center justify-center text-gray-400">กำลังโหลดข้อมูลร้าน...</div>;
  }
    ...
    if (guestSuccess) {
      return (
        <div className="min-h-screen bg-[#FDFDFD] flex flex-col items-center justify-center p-6 text-center font-sans">
          ...
          <button onClick={() => { setGuestSuccess(false); setFormData(initialForm); setSlipPreview(''); setSlipFile(null); }} className="bg-black text-white px-8 py-3.5 rounded-full font-medium active:scale-95 transition-transform shadow-lg shadow-black/10">
            สั่งเพิ่มอีกออเดอร์
          </button>
        </div>
      );
    ...
    return (
      <div className="min-h-screen bg-[#FDFDFD] pb-10 font-sans selection:bg-gray-200 text-gray-900 flex flex-col relative">
        <div className="pt-12 pb-6 px-6 max-w-lg mx-auto w-full text-center">
          ...
          <h1 className="text-2xl font-semibold tracking-tight mb-2">สั่งจองล่วงหน้า</h1>
        ...
        <div className="px-5 max-w-md mx-auto w-full flex-1 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <input type="text" className="w-full bg-white border border-gray-200/80 rounded-[20px] px-5 py-4 text-sm focus:outline-none focus:ring-2 focus:ring-black/5 transition-all shadow-sm" placeholder="ชื่อ-นามสกุล *" value={formData.name} onChange={e => setFormData({ ...formData, name: e.target.value })} />
              <input type="tel" className="w-full bg-white border border-gray-200/80 rounded-[20px] px-5 py-4 text-sm focus:outline-none focus:ring-2 focus:ring-black/5 transition-all shadow-sm" placeholder="เบอร์โทรศัพท์ติดต่อ *" value={formData.phone} onChange={e => setFormData({ ...formData, phone: e.target.value })} />
            </div>
          </div>
                ...
                {activeDates.map(dateObj => {
                  ...
                  return (
                    <button key={dateObj.id} onClick={() => setFormData({...formData, pickupDate: dateObj.date})}
                      className={`p-3 rounded-[20px] text-left border transition-all ${isSelected ? 'border-black bg-black text-white shadow-md' : 'border-gray-200 bg-white text-gray-600 hover:border-gray-300'}`}>
                      <div className="font-semibold text-sm">{dateObj.label}</div>
                      <div className={`text-xs mt-1 ${isSelected ? 'text-gray-300' : 'text-gray-400'}`}>{shortDate}</div>
                    </button>
                  );
                })}
              </div>
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-end pl-1">
              ...
              <button onClick={() => setFormData({ ...formData, items: [...formData.items, { menu: '', qty: '' }] })} className="text-xs font-medium text-black bg-gray-100 px-3 py-1 rounded-full active:scale-95">
                + เพิ่ม
              </button>
            </div>
            ...
            <div className="space-y-3 bg-white border border-gray-100 p-2 rounded-[24px] shadow-sm">
              {formData.items.map((item, i) => (
                <div key={i} className="flex gap-2 p-1">
                  <div className="flex-1 relative">
                    <select className="w-full bg-gray-50 border-none rounded-2xl px-4 py-3.5 text-sm outline-none appearance-none font-medium text-gray-700" value={item.menu} onChange={e => {
                  ...
                  <input type="number" min="1" className="w-20 bg-gray-50 border-none rounded-2xl px-2 py-3.5 text-center text-sm outline-none font-medium text-gray-700 placeholder:font-normal" placeholder="จำนวน" value={item.qty} onChange={e => {
                    const newItems = [...formData.items]; newItems[i].qty = e.target.value; setFormData({ ...formData, items: newItems });
                  }} />
                  
                  {formData.items.length > 1 && (
                    <button onClick={() => setFormData({...formData, items: formData.items.filter((_, idx) => idx !== i)})} className="w-12 flex items-center justify-center text-gray-400 hover:text-red-500 bg-gray-50 rounded-2xl active:bg-gray-100">
                      <Trash2 size={16}/>
                    </button>
                  )}
                </div>
              ...
              {calculatedTotal > 0 && (
                <div className="flex justify-between items-center px-4 py-3 bg-gray-50 rounded-2xl mt-2 border border-gray-100">
                  <span className="text-sm font-semibold text-gray-600">ยอดชำระทั้งหมด:</span>
                  <span className="text-lg font-bold text-black">฿{calculatedTotal.toLocaleString()}</span>
                </div>
              )}
            </div>
          </div>
          ...
          {calculatedTotal > 0 && settings?.promptpayId && (
            <div className="space-y-4">
              ...
              <div className="bg-white border border-gray-200/80 rounded-[24px] p-6 text-center shadow-sm">
                <img src={qrUrl} alt="PromptPay QR Code" className="mx-auto w-48 h-48 object-contain mb-4 rounded-xl border border-gray-100" />
                <p className="text-sm font-medium text-gray-800">พร้อมเพย์: {settings.promptpayId}</p>
                <p className="text-xs text-gray-500 mt-1">จำนวนเงิน <span className="font-bold text-black">฿{calculatedTotal.toLocaleString()}</span></p>
                ...
                <div className="mt-6 pt-5 border-t border-dashed border-gray-200">
                  ...
                  {!slipPreview ? (
                    <button onClick={() => fileInputRef.current.click()} className="w-full flex items-center justify-center gap-2 py-3 bg-blue-50 text-blue-600 rounded-xl text-sm font-semibold active:scale-95 transition-transform border border-blue-100">
                    ...
                    <div className="relative inline-block w-full">
                      ...
                      <button onClick={() => { setSlipPreview(''); setSlipFile(null); }} className="absolute top-2 right-2 p-1.5 bg-black/50 text-white rounded-full backdrop-blur-md">
                        <X size={14} />
                      </button>
          ...
          <div className="space-y-4">
            ...
            <textarea className="w-full bg-white border border-gray-200/80 rounded-[20px] px-5 py-4 text-sm focus:outline-none focus:ring-2 focus:ring-black/5 transition-all shadow-sm min-h-[80px] resize-none placeholder:text-gray-400" placeholder="ระบุความต้องการพิเศษ (ถ้ามี)" value={formData.note} onChange={e => setFormData({ ...formData, note: e.target.value })} />
          </div>

          <div className="pt-4 pb-8">
            <button onClick={handleSaveOrder} disabled={activeDates.length === 0 || isSubmitting} className="w-full bg-black text-white py-4 rounded-[20px] font-semibold text-sm shadow-xl shadow-black/10 active:scale-95 transition-all disabled:opacity-50 flex items-center justify-center gap-2">
              {isSubmitting ? <RefreshCw size={18} className="animate-spin" /> : <>ส่งคำสั่งจอง <ChevronLeft size={16} className="rotate-180" /></>}
            </button>
          </div>
        </div>
        ...
        <div className="py-6 text-center">
          <button onClick={() => setShowAdminLogin(true)} className="p-3 text-gray-300 hover:text-gray-400 transition-colors">
            <Lock size={14} />
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans max-w-md mx-auto border-x border-gray-100 relative">
      ...
      <div className="bg-white px-5 py-4 sticky top-0 z-20 border-b border-gray-100 flex justify-between items-center shadow-sm">
        <div>
          <h1 className="text-lg font-semibold text-gray-900 tracking-tight">Admin System</h1>
        </div>
        <button onClick={() => setRole('guest')} className="text-xs bg-gray-100 text-gray-600 px-3 py-1.5 rounded-full font-medium active:scale-95">สลับเป็นลูกค้า</button>
      </div>
      ...
      <div className="flex bg-white border-b border-gray-100 px-1 pt-1">
        {[
          ...
          <button key={t.id} onClick={() => setTab(t.id)} className={`flex-1 flex flex-col items-center py-3 gap-1 border-b-2 transition-all ${tab === t.id ? 'border-black text-black' : 'border-transparent text-gray-400 hover:text-gray-600'}`}>
          ...
          <div className="space-y-6 animate-in fade-in">
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-white p-4 rounded-3xl border border-gray-100 shadow-sm flex flex-col justify-between h-28">
                ...
                <div className="text-3xl font-semibold text-orange-500">{upcomingOrders.length}</div>
              </div>
              <div className="bg-white p-4 rounded-3xl border border-gray-100 shadow-sm flex flex-col justify-between h-28">
                <div className="text-xs text-gray-500 font-medium">ออเดอร์ทั้งหมด</div>
                <div className="text-3xl font-semibold text-gray-900">{orders.filter(o => o.status !== 'cancel').length}</div>
              </div>
              <div className="bg-white p-4 rounded-3xl border border-gray-100 shadow-sm flex flex-col justify-between h-28 col-span-2">
                <div className="text-xs text-gray-500 font-medium flex justify-between">
                  ...
                  {unpaidOrders.length > 0 && <span className="text-red-500">รอโอน {unpaidOrders.length} บิล</span>}
                </div>
                <div className="text-3xl font-semibold text-green-600">฿{totalDeposit.toLocaleString()}</div>
              </div>
            </div>
            
            <button onClick={handleSyncToSheets} disabled={syncing} className={`w-full py-4 rounded-3xl font-semibold flex items-center justify-center gap-2 transition-all shadow-sm text-sm ${syncing ? 'bg-green-50 text-green-700 border border-green-200' : 'bg-black text-white active:scale-95'}`}>
          ...
          <div className="space-y-4 animate-in fade-in">
            <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide px-1">
              {[{ id: 'all', label: 'ทั้งหมด' }, { id: 'pending', label: 'รอรับ' }, { id: 'done', label: 'รับแล้ว' }, { id: 'cancel', label: 'ยกเลิก' }].map(f => (
                <button key={f.id} onClick={() => setFilterStatus(f.id)} className={`px-4 py-2 rounded-full text-xs font-semibold whitespace-nowrap transition-colors ${filterStatus === f.id ? 'bg-black text-white shadow-md' : 'bg-white text-gray-500 border border-gray-200'}`}>
                  {f.label}
                </button>
              ))}
            </div>
        ...
        {tab === 'settings' && (
          <div className="space-y-6 animate-in fade-in">
            ...
            <div className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm space-y-3">
              <h3 className="text-sm font-semibold text-gray-900 flex items-center gap-2"><Receipt size={16}/> บัญชีรับเงิน (PromptPay)</h3>
              <p className="text-xs text-gray-500">ระบุเบอร์โทรหรือบัตรประชาชน เพื่อสร้าง QR Code ให้ลูกค้าแสกนจ่ายอัตโนมัติ</p>
              <input type="text" className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3 text-sm outline-none focus:border-black transition-colors" value={settings?.promptpayId || ''} onChange={e => handleUpdateSettings({...settings, promptpayId: e.target.value})} placeholder="08XXXXXXXX หรือ 1XXXXXXXXXXXX" />
            </div>

            {/* Announcement */}
            <div className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm space-y-3">
              <h3 className="text-sm font-semibold text-gray-900">ประกาศหน้าร้าน</h3>
              <input type="text" className="w-full bg-gray-50 border border-gray-200 rounded-2xl px-4 py-3 text-sm outline-none focus:border-black transition-colors" value={settings?.announcement || ''} onChange={e => handleUpdateSettings({...settings, announcement: e.target.value})} placeholder="เช่น เปิดรับออเดอร์ถึง 18.00 น." />
            </div>
            ...
            <div className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm space-y-4">
              <div className="flex justify-between items-center">
                ...
                }} className="text-xs bg-gray-100 font-semibold px-3 py-1.5 rounded-full">+ เพิ่มรอบ</button>
              </div>
              <div className="space-y-3">
                {settings?.pickupDates?.map((d, index) => (
                  <div key={d.id} className="bg-gray-50 p-3 rounded-2xl border border-gray-200 flex flex-col gap-3">
                    <div className="flex gap-2">
                      <input type="date" className="bg-white border border-gray-200 rounded-xl px-3 py-2 text-xs flex-1 outline-none" value={d.date} onChange={e => {
                      ...
                      <input type="text" className="bg-white border border-gray-200 rounded-xl px-3 py-2 text-xs flex-1 outline-none" value={d.label} placeholder="ชื่อรอบ" onChange={e => {
                    ...
                    <div className="flex justify-between items-center px-1">
                      ...
                      }} className="text-xs text-red-500 font-medium p-1">ลบ</button>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="bg-white p-5 rounded-3xl border border-gray-100 shadow-sm space-y-4">
              <div className="flex justify-between items-center">
                ...
                }} className="text-xs bg-gray-100 font-semibold px-3 py-1.5 rounded-full">+ เพิ่มเมนู</button>
              </div>
              <div className="space-y-2">
                {settings?.menus?.map((m, index) => (
                  <div key={m.id} className="flex gap-2 items-center">
                    <input type="text" className="bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-sm flex-1 outline-none focus:border-black" value={m.name} placeholder="ชื่อเมนู" onChange={e => {
                      const newMenus = [...settings.menus]; newMenus[index].name = e.target.value; handleUpdateSettings({...settings, menus: newMenus});
                    }} />
                    <input type="number" className="bg-gray-50 border border-gray-200 rounded-xl px-3 py-2.5 text-sm w-20 outline-none focus:border-black text-center" value={m.price} placeholder="ราคา" onChange={e => {
                    ...
                    }} className="p-2.5 text-gray-400 hover:text-red-500 bg-gray-50 rounded-xl"><Trash2 size={16}/></button>
                  </div>
                ))}
              </div>
            </div>
      ...
      {tab === 'orders' && (
        <button onClick={() => openModal()} className="absolute bottom-6 right-6 w-14 h-14 bg-black text-white rounded-full shadow-xl flex items-center justify-center active:scale-90 transition-transform z-10">
          <Plus size={28} />
        </button>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
          <div className="bg-[#FDFDFD] w-full max-w-md rounded-t-[32px] sm:rounded-[32px] h-[90vh] sm:h-auto sm:max-h-[90vh] flex flex-col shadow-2xl animate-in slide-in-from-bottom-10 sm:zoom-in-95 duration-200">
            ...
            <div className="p-5 border-b border-gray-100 flex justify-between items-center bg-white rounded-t-[32px] sm:rounded-[32px] shrink-0">
              <h2 className="text-lg font-semibold text-gray-900">{editId ? 'แก้ไขออเดอร์' : 'เพิ่มออเดอร์ใหม่'}</h2>
              <button onClick={() => setIsModalOpen(false)} className="p-2 bg-gray-50 hover:bg-gray-100 rounded-full text-gray-500 active:scale-95 transition-colors"><X size={18}/></button>
            </div>
            ...
            <div className="p-6 overflow-y-auto space-y-5 flex-1">
              ...
              <div className="space-y-4">
                <input type="text" className="w-full bg-white border border-gray-200 rounded-2xl px-4 py-3.5 text-sm outline-none focus:border-black" placeholder="ชื่อลูกค้า" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                <div className="flex gap-3">
                  <input type="tel" className="flex-1 bg-white border border-gray-200 rounded-2xl px-4 py-3.5 text-sm outline-none focus:border-black" placeholder="เบอร์โทร" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} />
                  <input type="date" className="flex-1 bg-white border border-gray-200 rounded-2xl px-4 py-3.5 text-sm outline-none focus:border-black text-gray-600" value={formData.pickupDate} onChange={e => setFormData({...formData, pickupDate: e.target.value})} />
                </div>
              </div>
              
              <div className="bg-white p-4 rounded-[24px] border border-gray-200 space-y-3">
                <label className="block text-xs font-semibold text-gray-900 ml-1">รายการเมนู</label>
                {formData.items.map((item, i) => (
                  <div key={i} className="flex gap-2">
                    <select className="flex-1 bg-gray-50 border border-gray-100 rounded-xl px-3 py-3 text-sm outline-none" value={item.menu} onChange={e => {
                      const newItems = [...formData.items]; newItems[i].menu = e.target.value; setFormData({ ...formData, items: newItems });
                    }}>
                      <option value="">เลือกเมนู...</option>
                      {settings?.menus?.map(m => <option key={m.id} value={m.name}>{m.name}</option>)}
                    </select>
                    <input type="number" min="1" className="w-20 bg-gray-50 border border-gray-100 rounded-xl px-2 py-3 text-center text-sm outline-none" placeholder="จำนวน" value={item.qty} onChange={e => {
                      const newItems = [...formData.items]; newItems[i].qty = e.target.value; setFormData({ ...formData, items: newItems });
                    }} />
                    <button onClick={() => setFormData({...formData, items: formData.items.filter((_, idx) => idx !== i)})} className="p-3 text-gray-400 hover:text-red-500 bg-gray-50 rounded-xl"><Trash2 size={16}/></button>
                  </div>
                ))}
                <button onClick={() => setFormData({ ...formData, items: [...formData.items, { menu: '', qty: '' }] })} className="text-xs text-black font-semibold w-full py-3 mt-2 bg-gray-50 hover:bg-gray-100 rounded-xl transition-colors">
                  + เพิ่มรายการ
                </button>
              </div>

              <div className="grid grid-cols-2 gap-3">
                <input type="number" className="w-full bg-white border border-gray-200 rounded-2xl px-4 py-3.5 text-sm outline-none focus:border-black" value={formData.deposit} onChange={e => setFormData({...formData, deposit: e.target.value})} placeholder="ยอดที่ยืนยันแล้ว (฿)" />
                <select className="w-full bg-white border border-gray-200 rounded-2xl px-4 py-3.5 text-sm outline-none focus:border-black font-medium" value={formData.status} onChange={e => setFormData({...formData, status: e.target.value})}>
              ...
              <textarea className="w-full bg-white border border-gray-200 rounded-2xl px-4 py-3.5 text-sm outline-none focus:border-black resize-none h-24" value={formData.note} onChange={e => setFormData({...formData, note: e.target.value})} placeholder="หมายเหตุเพิ่มเติม..." />
            </div>
            
            <div className="p-5 bg-white border-t border-gray-100 pb-8 sm:pb-5 flex gap-3 shrink-0">
              {editId && (
                <button onClick={() => handleDelete(editId)} className="px-5 py-4 bg-red-50 text-red-600 rounded-2xl font-semibold text-sm flex-none active:scale-95 transition-transform">ลบ</button>
              )}
              <button onClick={handleSaveOrder} disabled={isSubmitting} className="flex-1 py-4 bg-black text-white rounded-2xl font-semibold text-sm shadow-xl shadow-black/10 active:scale-95 transition-transform flex justify-center items-center">
...
function OrderCard({ order, onEdit }) {
  ...
  const statusColors = isDone ? 'bg-green-50 text-green-700 border-green-200' : isCancel ? 'bg-red-50 text-red-600 border-red-200' : 'bg-orange-50 text-orange-700 border-orange-200';
    ...
    <div className={`bg-white p-5 rounded-3xl border shadow-sm relative transition-all ${isCancel ? 'border-gray-100 opacity-70' : 'border-gray-200'}`}>
      ...
      <div className="flex justify-between items-end pt-3 border-t border-gray-100">
        <div className="flex items-center gap-2">
          ...
          {order.slipUrl && (
            <a href={order.slipUrl} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-[10px] bg-blue-50 text-blue-600 px-2 py-1 rounded-md font-medium border border-blue-100">
              <ImageIcon size={12}/> ดูสลิป
            </a>
          )}
        </div>
        <button onClick={onEdit} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-full transition-colors active:scale-95">
          <Edit3 size={12} /> แก้ไข
        </button>
      </div>
      {order.note && (
